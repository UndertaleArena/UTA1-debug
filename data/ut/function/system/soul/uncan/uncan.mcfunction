...
clear @s carrot_on_a_stick[custom_data~{soul:1b,canned:1b}] 1

###Summon Item
data modify block 255 0 255 Items set value [{Slot:0b,id:"minecraft:carrot_on_a_stick",count:1b}]
execute store result score #cmd soul run data get storage soul SoulInv[0].components."minecraft:custom_model_data".floats[0]
scoreboard players remove #cmd soul 10
execute store result storage soul SoulInv[0].components."minecraft:custom_model_data".floats[0] int 1 run scoreboard players get #cmd soul
