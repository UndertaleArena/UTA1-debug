...
execute at @e[tag=atker] as @e[tag=launched,scores={credit=-18..-10}] run function ut:lobby/credit/a/sm_s
tag @e[tag=atker] remove atker

execute if score credit_state info matches 1 run schedule function ut:lobby/credit/e/d3 30t